...
def opt_quintic_polynomial_planner(sx: float, sy: float, syaw: float, sv: float, sa: float,
                                   gx: float, gy: float, max_ve: float,
                                   dt: float=0.1, time_horizon: float = 3):
    traj_model = gurobipy.Model()
    traj_model.params.NonConvex = 2
    traj_model.setParam('NonConvex', 2)
    xve = traj_model.addVar(lb=-max_ve, ub=max_ve, vtype=gurobipy.GRB.CONTINUOUS, name="xve")
    xae = traj_model.addVar(lb=-3, ub=2, vtype=gurobipy.GRB.CONTINUOUS, name="xae")
    yve = traj_model.addVar(lb=-max_ve, ub=max_ve, vtype=gurobipy.GRB.CONTINUOUS, name="yve")
    yae = traj_model.addVar(lb=-3, ub=2, vtype=gurobipy.GRB.CONTINUOUS, name="yae")
    # cos_gyaw = traj_model.addVar(lb=-1, ub=1, vtype=gurobipy.GRB.CONTINUOUS, name="cos_gyaw")
    # sin_gyaw = traj_model.addVar(lb=syaw-1, ub=1, vtype=gurobipy.GRB.CONTINUOUS, name="sin_gyaw")
    traj_model.update()
    ...
    xa3_yaw_coe = -(8*time_horizon)/(2*time_horizon**3)
    ...
    xa4_yaw_coe = (7*time_horizon)/time_horizon**4
    ...
    xa5_yaw_coe = -(6*time_horizon)/(2*time_horizon**5)
    # x_ae_coe = -(-time_horizon**2*math.cos(gyaw))/(2*time_horizon**3)
    x_yaw_coe = -(-time_horizon**2)/(2*time_horizon**3)
    ...
    ya3_yaw_coe = -(8*time_horizon)/(2*time_horizon**3)
    ...
    ya4_yaw_coe = (7*time_horizon)/time_horizon**4
    ...
    ya5_yaw_coe = -(6*time_horizon)/(2*time_horizon**5)
    # y_ae_coe = -(-time_horizon**2*math.sin(gyaw))/(2*time_horizon**3)
    y_yaw_coe = -(-time_horizon**2)/(2*time_horizon**3)
    ...
    for i in range(1, 31):
        t = i*0.1
        cur_x = xa0 + xa1*t + xa2*t**2 + (xa3_part+xa3_yaw_coe*xve+x_yaw_coe*xae)*t**3 + \
        (xa4_part+xa4_yaw_coe*xve+x_yaw_coe*xae)*t**4 + (xa5_part+xa5_yaw_coe*xve+x_yaw_coe*xae)*t**5
        cur_y = ya0 + ya1*t + ya2*t**2 + (ya3_part+ya3_yaw_coe*yve+y_yaw_coe*yae)*t**3 + \
        (ya4_part+ya4_yaw_coe*yve+y_yaw_coe*yae)*t**4 + (ya5_part+ya5_yaw_coe*yve+y_yaw_coe*yae)*t**5
        # cur_v = ((cur_x-start_x)*(cur_x-start_x)+(cur_y-start_y)*(cur_y-start_y)/0.01)
        # cur_a = (cur_v-start_v)/0.01
        obj_express.add((cur_x-start_x)*(cur_x-start_x)+(cur_y-start_y)*(cur_y-start_y))
        # if i == 30:
        #     obj_express.add(10*(cur_v-sv))
        start_x = cur_x
        start_y = cur_y
        # start_v = cur_v
        # start_a = cur_a
    ...
    print(xve.x, xae.x, yve.x, yae.x)
    
    return math.sqrt(xve.x**2+yve.x**2), math.sqrt(xae.x**2+yae.x**2), math.atan2(yve.x, xve.x)
...
if __name__ == '__main__':
    ...
    for file_name in scene_list:
        ...
        for scene_name, case_id, track_id in tqdm(target_veh_list):
            ...
            last_vel = math.sqrt(track_full_v[-1][0] ** 2 + track_full_v[-1][1] ** 2)
            front_vel = math.sqrt(track_full_v[-2][0] ** 2 + track_full_v[-2][1] ** 2)
            a_e = (last_vel - front_vel) / 0.1
            
            
            ve, ae, yaw = opt_quintic_polynomial_planner(
            ...
            print("GT vel = {}, acc = {}, yaw = {}".format(last_vel, a_e, track_full_yaw[-1]))
            print("pred vel = {}, acc = {}, yaw = {}".format(ve, ae, yaw))
