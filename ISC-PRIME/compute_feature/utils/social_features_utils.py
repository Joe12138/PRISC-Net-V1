...
from compute_feature.utils.baseline_config import (
    PADDING_TYPE,
...
class SocialFeaturesUtils:
    ...
    def compute_velocity(self, track_df: pd.DataFrame) -> List[float]:
        ...
        timestamp = track_df["frame_id"].values
        vel_x, vel_y = zip(*[(
                (x_coord[i] - x_coord[i - 1]) / (float(timestamp[i]) - float(timestamp[i - 1])),
                (y_coord[i] - y_coord[i - 1]) / (float(timestamp[i]) - float(timestamp[i - 1]))) for i in range(1, len(timestamp))
            ]
        )
    ...
    def get_is_track_stationary(self, track_df: pd.DataFrame) -> bool:
        ...
        ## Now the maximal vel is used to check
        vel_stat = self.compute_velocity(track_df)
        threshold_vel = np.max(vel_stat)
    ...
    def fill_track_lost_in_middle(
        ...
        """
        curr_idx = 0
        padding_flag = np.ones((len(seq_timestamps),1), dtype=int)
        filled_track = np.empty((0, track_array.shape[1]))
        for id, timestamp in enumerate(seq_timestamps):
            filled_track = np.vstack((filled_track, track_array[curr_idx]))
            if timestamp in track_array[:, raw_data_format["frame_id"]]:
                curr_idx += 1
                padding_flag[id] = 0
        filled_track = np.hstack((filled_track, padding_flag))

        return filled_track

    def fill_track_lost_on_sides(self,
        ...
        """
        Edge padding in front and rear to satisfy the length of observation
        Args:
            track_vals (numpy array): tracking data from dataframe
            seq_timestamps (numpy array): All timestamps in the sequence
            track_timestamps: All tracking timestamps from dataframe
            obs_len (int): Length of observed trajectory obs_len:
        Returns:
                padded_track_array (numpy array): Track data padded in front and back, with padding flag column
        """
        # Compare timestamps to find start and end index of the track in the sequence
        start_idx = np.where(seq_timestamps == track_timestamps[0])[0][0]
        end_idx = np.where(seq_timestamps == track_timestamps[-1])[0][0]
        # Edge padding in front and rear
        padded_track_array = np.pad(track_vals,
                                    ((start_idx, obs_len - end_idx - 1),
                                    (0, 0)), "edge")

        padding_flag = np.ones((padded_track_array.shape[0], 1), dtype=int)
        padding_flag[start_idx:end_idx+1,:] = 0
        padded_track_array = np.hstack((padded_track_array, padding_flag))
    ...
    def pad_track(
        ...
        Args:
            ...
            raw_data_format (Dict): Format of the sequence
            padding_column_flag (bool): Whether show the padding flag in output
        Returns:
                padded_track_array (numpy array): Track data padded in front and back

        """
        track_vals = track_df.values
        track_timestamps = track_df["frame_id"].values

        if self.PADDING_TYPE == "REPEAT":
            # Edge padding in front and rear, i.e., repeat the first and last coordinates
            padded_track_array = self.fill_track_lost_on_sides(track_vals,
                                                               seq_timestamps=seq_timestamps,
                                                               track_timestamps=track_timestamps,
                                                               obs_len=obs_len )
            if padded_track_array.shape[0] < obs_len:
                # Handling rare case, tracking lost in middle with same tracking id
                padded_track_array = self.fill_track_lost_in_middle(padded_track_array[:,:-1],
                                                                    seq_timestamps,
                                                                    raw_data_format )
        ...
        if padding_column_flag:
            ...
            return padded_track_array[:,:-1]

    def filter_tracks(self, seq_df: pd.DataFrame, track_id: int, obs_len: int,
                      raw_data_format: Dict[str, int],
                      filter_stationary_tracks: bool,
                      padding_column_flag: bool) -> np.ndarray:
        ...
        """
        if padding_column_flag:
            social_tracks = np.empty((0, obs_len, len(raw_data_format) + 1))
        else:
            social_tracks = np.empty((0, obs_len, len(raw_data_format)))
        # Timestamps in the sequence
        seq_timestamps = np.unique(seq_df["frame_id"].values)

        # Track groups (adding tracked neighbours one by one)
        df_groups = seq_df.groupby("track_id")
        for group_name, group_data in df_groups:
            # Check if the track is long enough (total timestep)
            if len(group_data) < self.EXIST_THRESHOLD:
                continue

            # Skip if it's agent track
            if group_data["track_id"].iloc[0] == track_id:
                continue

            # Check if the track is stationary
            if filter_stationary_tracks and self.get_is_track_stationary(group_data):
                continue
            ...
            padded_track_array = padded_track_array.reshape((1, obs_len, -1))
    ...
    # Make it a single function (only the frames within obs_len would be counted in the social features)
    def get_social_tracks(
            self,
            df: pd.DataFrame,
            track_id: int,
            obs_len: int,
            ...
            padding_column_flag: bool = True

    ) -> np.ndarray:
        agent_ts = np.sort(np.unique(df["frame_id"].values))

        if agent_ts.shape[0] == obs_len:
            df_obs = df
        else:
            # Get obs dataframe and agent track
            df_obs = df[df["frame_id"] < agent_ts[obs_len]]
            assert (np.unique(df_obs["frame_id"].values).shape[0] == obs_len
                    ), "Obs len mismatch"

        social_tracks_obs = self.filter_tracks(df_obs, track_id, obs_len,
                                               raw_data_format, filter_stationary_tracks,
                                               padding_column_flag=padding_column_flag)
        return social_tracks_obs
    ...
    def compute_social_features(
        ...
        social_tracks_obs,_ = self.get_social_tracks(
            df, track_id, obs_len, raw_data_format, filter_stationary_tracks=True)
