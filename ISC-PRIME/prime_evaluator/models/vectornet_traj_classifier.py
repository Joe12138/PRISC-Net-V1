...
class TrajClassifier(nn.Module):
    def __init__(self, args, device):
        ...
        self.combin_size = self.traj_enc_size * 3 + 64

        self.vector_net_train_set = InteractionInMem(os.path.join(args.data_root, "train_intermediate"))
        self.vector_net_eval_set = InteractionInMem(os.path.join(args.data_root, "val_intermediate"))

        if os.path.exists(os.path.join(args.data_root, "train_intermediate", "processed", "key.json")):
            f = open(os.path.join(args.data_root, "train_intermediate", "processed", "key.json"), "r", encoding="UTF-8")
            self.train_idx_dict = json.load(f)
            f.close()
        else:
            self.train_idx_dict = {}
            self.get_idx_dict(data_root=args.data_root, mode="train")
        
        if os.path.exists(os.path.join(args.data_root, "val_intermediate", "processed", "key.json")):
            f = open(os.path.join(args.data_root, "val_intermediate", "processed", "key.json"), "r", encoding="UTF-8")
            self.val_idx_dict = json.load(f)
            f.close()
        else:
            self.val_idx_dict = {}
            self.get_idx_dict(data_root=args.data_root, mode="val")
        ...
        # VectorNet
        self.vector_net = VectorNetBackbone(
            in_channels=self.vector_net_train_set.num_features if hasattr(self.vector_net_train_set, "num_features") else self.vector_net_eval_set.num_features,
    ...
    def get_idx_dict(self, data_root: str, mode: str = "train"):
        if mode == "train":
            seq_list = self.vector_net_train_set.data.seq_id
        else:
            seq_list = self.vector_net_eval_set.data.seq_id
        ...
        with open(os.path.join(data_root, f"{mode}_intermediate", "processed", "key.json"), "w", encoding="UTF-8") as f:
            if mode == "train":
                json.dump(self.train_idx_dict, f)
            else:
                json.dump(self.val_idx_dict, f)

    def forward(self, sample_batched, mode="train"):
        device = torch.device("cuda:0")
        seq_ids_list = sample_batched['seq_ids']
        ...
        for k in range(batch_size):
            file_name = seq_ids_list[k]
            key_str = file_name[4:-4]
            if mode == "train":
                idx = self.train_idx_dict[key_str]
                vector_net_data = self.vector_net_train_set.get(idx).cuda()
            else:
                idx = self.val_idx_dict[key_str]
                vector_net_data = self.vector_net_eval_set.get(idx).cuda()
            vector_net_data.num_graphs = 1
            vector_net_data.batch = None
            ...
            global_feat, aux_out, aux_gt = self.vector_net(vector_net_data, mode="no_normal")
            target_feat = global_feat[:, 0].unsqueeze(1)
            target_feat = target_feat.repeat(agent_obs_part.shape[0], 1, 1)

            under_this_sample = torch.cat((agent_obs_part, agent_interaction_part, agent_pred_part, target_feat), dim=-1)
