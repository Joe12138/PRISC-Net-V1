...
from target_prediction.model.layers.scoring_and_selection import distance_metric


class TargetPredLoss(nn.Module):
    def __init__(self, aux_loss=False, device=torch.device("cuda")):
        super(TargetPredLoss, self).__init__()
    ...
    def forward(self, pred_dict, gt_dict, aux_pred=None, aux_gt=None):
        """
        Forward function
        :param pred_dict: The dictionary containing model prediction.
            {
        ...
        :param gt_dict: The dictionary containing the prediction gt.
            {
                "target_prob": the one-hot gt of target candidate,
                "offset":      the gt for the offset of the nearest target candidate to the targte position
        ...
        if self.aux_loss:
            ...
            assert aux_pred.size() == aux_gt.size(), "[ParameterGeneratorLoss]: The dim of prediction and ground truth " \
                                                     "don't match!"
