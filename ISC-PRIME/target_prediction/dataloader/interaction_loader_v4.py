...
class InteractionInMem(InMemoryDataset):
    ...
    def _get_x(data_seq):
        ...
        for _, [feat, has_obs] in enumerate(zip(traj_feats, traj_has_obss)):
            ...
            feats = np.vstack([feats, np.hstack([xy_s, vec, step[has_obs][:-1], traffic_ctrl, is_turn,
                                                 is_intersect, speed_limit, polyline_id])])
            traj_cnt += 1
        ...
        speed_limit = graph["speed_limit"].reshape(-1, 1)
        lane_idcs = graph["lane_idcs"].reshape(-1, 1) + traj_cnt
        steps = np.zeros((len(lane_idcs), 1))
        feats = np.vstack([feats, np.hstack([ctrs, vec, steps, traffic_ctrl, is_turn, is_intersect, speed_limit, lane_idcs])])
    ...
    def get_speed_candidate(self, raw_data):
        track_id = raw_data["track_id"].values[0].astype(int)
        full_info = raw_data["full_info_dict"].values[0][track_id]
        ...
        speed_array = np.hypot(vx_array, vy_array)

        cur_speed = speed_array[9][0]

        speed_candts = np.linspace(max(0, cur_speed-5), cur_speed+5, 21)

        return np.asarray(speed_candts).reshape((-1, 1)), speed_array
    ...
    def process(self):
        ...
        for raw_path in tqdm(self.raw_paths, desc="Loading Raw Data..."):
            ...
            if self.split == "test":
                ...
                tar_candts_gt, tar_offset_gt = self.get_candidate_gt_v2(target_candidate=tar_candts,
        ...
        for ind, raw_path in enumerate(tqdm(self.raw_paths, desc="Transforming the data to GraphData...")):
            ...
            speed_candts, speed_array = self.get_speed_candidate(raw_data)
            speed_candts_gt, speed_offset_gt = self.get_yaw_vel_candidate_gt_v2(speed_candts, speed_array[-1])
            yaw_candts_gt, yaw_offset_gt = self.get_yaw_vel_candidate_gt_v2(yaw_candts, yaw_array[-1])
            vx_candts_gt, vx_offset_gt = self.get_yaw_vel_candidate_gt_v2(vx_candts, vx_array[-1])
            vy_candts_gt, vy_offset_gt = self.get_yaw_vel_candidate_gt_v2(vy_candts, vy_array[-1])

            # candidate length information
            target_offset_gt_len = torch.from_numpy(offset_gt_dict[raw_path]).float().shape[0]
            yaw_offset_gt_len = torch.from_numpy(yaw_offset_gt).float().shape[0]
            vx_offset_gt_len = torch.from_numpy(vx_offset_gt).float().shape(0)
            vy_offset_gt_len = torch.from_numpy(vy_offset_gt).float().shape(0)
            speed_offset_gt_len = torch.from_numpy(speed_offset_gt).float().shape[0]
            ...
            graph_input = GraphData(
                ...
                speed_candts=torch.from_numpy(speed_candts).float(),
                ...
                speed_array=torch.from_numpy(speed_array).float(),

                yaw_candts_gt=torch.from_numpy(yaw_candts_gt).bool(),
                vx_candts_gt=torch.from_numpy(vx_candts_gt).bool(),
                vy_candts_gt=torch.from_numpy(vy_candts_gt).bool(),
                speed_candts_gt=torch.from_numpy(speed_candts_gt).bool(),
                ...
                speed_offset_gt=torch.from_numpy(speed_offset_gt).float(),

                target_offset_gt_len=torch.tensor([target_offset_gt_len]).int(),
                yaw_offset_gt_len=torch.tensor([yaw_offset_gt_len]).int(),
                vx_offset_gt_len=torch.tensor([vx_offset_gt_len]).int(),
                vy_offset_gt_len=torch.tensor([vy_offset_gt_len]).int(),
                speed_offset_gt_len=torch.tensor([speed_offset_gt_len]).int(),
