...
def train(gpu, args):
    train_set = TargetInteractionInMem(pjoin(args.data_root, "train_intermediate")).shuffle()
    eval_set = TargetInteractionInMem(pjoin(args.data_root, "val_intermediate"))
    ...
    trainer = YawVelTrainer(
        ...
        variable=args.predict_variable
    )
    ...
    for iter_epoch in range(args.n_epoch):
        ...
        if not min_eval_loss:
            ...
            trainer.save_yaw_model("best")

    trainer.save_yaw_model("final")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    variable = "vx"
    parser.add_argument("-d", "--data_root", required=False, type=str,
    ...
    parser.add_argument("-o", "--output_dir", required=False, type=str,
                        default=f"/home/joe/Desktop/Rule-PRIME/Model/{variable}_output",
    ...
    parser.add_argument("-e", "--n_epoch", type=int, default=150,
    ...
    parser.add_argument("-v", "--predict_variable", type=str, default=variable,
                        help="resume a model state for fine-tune")
